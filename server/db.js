...
const allMinions = new Array(10).fill(0).map(createMinion); // Array of objects
/*
This list represents all of the minions that the company employs
e.g.,
[
  {
    id: '1',
    name: 'Bob',
    weaknesses: 'Cannot do math, too honest',
    title: 'Intern',
    salary: 40000,
  },
  ...
...
*/
const allIdeas = new Array(10).fill(0).map(createIdea); // Array of objects
/*
This list represents the ideas that the minions have pitched
e.g.,
[
  {
    id: '1',
    name: 'Uber but for dogs',
    description: 'The name says it all!!!',
    weeklyRevenue: 123456,
    numWeeks: 10,
  },
  ...
]
*/
const allWork = allMinions.map(minion => createWork(minion.id)); // Array of objects
/*
This list represents the work that each minion does
e.g.,
[
  {
    id: '1',
...
/*
This list describes all of the meetings that have taken place
e.g.,
[
  {
    id: '1',
    ...
    day: 'Friday',
    note: 'Discussion about Uber but for dogs',
  },
  ...
]
*/
...
// Getting entire list of objects of certain model type
...
// Finding a specific obj in the database based on objects id property
const getFromDatabaseById = (modelType, id) => {
  const model = findDataArrayByName(modelType);
  if (model === null) {
    return null;
  }
  // .find returns element itself while .findIndex returns index itself
  // element or undefined vs index of -1
  return model.data.find((element) => {
    return element.id === id;
  });
}

// Adding a new object to the database, returns new object if successful, null if not
...
// Updating an object in the database, returns updated object if successful, null if not
...
// Deleting an object from the database, returns true if successful, false if not
...
// Deleting all objects from the database, returns empty array if successful, null if not
